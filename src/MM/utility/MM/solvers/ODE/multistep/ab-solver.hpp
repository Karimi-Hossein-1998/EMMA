...
inline SolverResults<Num> adams_bashforth(const ODESolverParameters<Num,MF,CBF>& Params)
{
    ...
    // size_t boot_strap{static_cast<size_t>(order-1)};
...
inline SolverResults<Num> adams_bashforth_solver(const ODESolverParameters<Num,MF,CBF>& params)
{
    return funcWrapper::adams_bashforth<false>(params);
}
template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> adams_bashforth_solver_callback(const ODESolverParameters<Num,MF,CBF>& params)
{
    return funcWrapper::adams_bashforth<true>(params);
}
// Basic interface wrapper for Adams-Bashforth-Moulton solver
template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline Matrix<Num> adams_bashforth_solver(
    MF              deriv,
    const Vec<Num>& y0,
    Num             t0,
    Num             t1,
...
)
{
    ...
    return adams_bashforth_solver_wrapper<false>(params).solution;
}
