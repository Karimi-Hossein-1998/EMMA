...
inline SolverResults<Num> rk4_38(const ODESolverParameters<Num,MF,CBF>& Params)
{
    ...
    Vec<Num> y(y0), y_temp(N,0.0), k1(N, 0.0), k2(N, 0.0), k3(N, 0.0), k4(N, 0.0);
    Num dt_third = dt / 3.0;
    Num dt_2_3   = dt_third * 2.0;
    Num dt_8     = dt / 8.0;
    Num dt_3_8   = dt_8 * 3.0;
...
template <bool EnableCallBack, FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_ralston(const ODESolverParameters<Num,MF,CBF>& Params)
{
    // Extract parameters for clarity
    const MF&   f  = Params.derivative;
    const CBF&  cb = Params.onStep;
    ...
    Vec<Num> y(y0), y_temp(N,0.0), k1(N, 0.0), k2(N, 0.0), k3(N, 0.0), k4(N, 0.0);
    Num c2 = 0.4, c3 = 0.45573725, c4 = 1.0;
    Num a21 = 0.4, a31 = 0.15875964, a32 = 0.29697761, a41 = 0.2181004, a42 = -3.05096516, a43 = 3.83286476;
    Num b1 = 0.17476028, b2 = -0.55148066, b3 = 1.2055356, b4 = 0.17118478;
	Num dtc2 = dt*c2, dtc3 = dt*c3;
    Num dta21 = dt*a21, dta31 = dt*a31, dta32 = dt*a32, dta41 = dt*a41, dta42 = dt*a42, dta43 = dt*a43;
    Num dtb1 = dt*b1, dtb2 = dt*b2, dtb3 = dt*b3, dtb4 = dt*b4;
    // Main integration loop
    OneStepSolverResult<Num> stepRes{
        .sol = y0,
        .timePoint = t0,
...
template <bool EnableCallBack, FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_gill(const ODESolverParameters<Num,MF,CBF>& Params)
{
    // Extract parameters for clarity
    const MF&   f  = Params.derivative;
    const CBF&  cb = Params.onStep;
    ...
    Num dt_half      = dt * 0.5;
    Num dt_sixth     = dt / 6.0;
    Num sqrt2        = std::sqrt(2.0);
    Num sqrt2inverse = sqrt2 / 2.0;
    Num dt_s2inverse = dt * sqrt2inverse;
    Num a            = sqrt2inverse - 0.5;
    Num dt_a         = dt * a;
    Num b            = 1.0 - sqrt2inverse;
    Num dt_b         = dt * b;
    Num c            = 1.0 + sqrt2inverse;
    Num dt_c         = dt * c;
    Num dy_2         = dt_sixth * b * 2.0;
    Num dy_3         = dt_sixth * c * 2.0;
    // Main integration loop
    OneStepSolverResult<Num> stepRes{
        .sol = y0,
        .timePoint = t0,
...
inline SolverResults<Num> rk4_38_solver(const ODESolverParameters<Num,MF,CBF>& Params)
{
    return funcWrapper::rk4_38<false>(Params);
}
template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_38_solver_callback(const ODESolverParameters<Num,MF,CBF>& Params)
{
    return funcWrapper::rk4_38<true>(Params);
}

template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_ralston_solver(const ODESolverParameters<Num,MF,CBF>& Params)
{
    return funcWrapper::rk4_ralston<false>(Params);
}
template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_ralston_solver_callback(const ODESolverParameters<Num,MF,CBF>& Params)
{
    return funcWrapper::rk4_ralston<true>(Params);
}

template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_gill_solver(const ODESolverParameters<Num,MF,CBF>& Params)
{
    return funcWrapper::rk4_gill<false>(Params);
}
template <FPNumber Num, MyFunc<Num> MF, CallBackFunc<Num> CBF>
inline SolverResults<Num> rk4_gill_solver_callback(const ODESolverParameters<Num,MF,CBF>& Params)
{
